function read_sched_file(file, file_directory)
    cd(file_directory) #change location to desired directory
    ...
    open(file) do f
        ...
        for line in eachline(file)
            ...
            if  (inWELSPECS == true) & (!occursin("WELSPECS", line)) & (!occursin("--", line))
                aux = split(line)
...
function write_sched_file(file, well_dict, file_directory)
    ...
    open(file) do io
        ...
        for line in eachline(io, keep=true) # keep = true so the new line isn't chomped
            ...
            if (inWELSPECS == true) & (string(line[1]) == "/") #if the WELSPECS section is over
                inWELSPECS = false
            end
            ...
            if (inCOMPDAT == true) & (string(line[1]) == "/") #if the WELSPECS section is over
                inCOMPDAT = false
            end
            ...
            if (inCOMPDAT == true) & (!occursin("COMPDAT", line)) & (!occursin("--", line))
                aux = split(line)
                well = aux[1]
                if haskey(well_dict,well) #sanity check
...
well_dict = read_sched_file("base.sched", pwd())
well_dict["'PWELL04'"][1] = 100
write_sched_file("base.sched", well_dict, pwd())
